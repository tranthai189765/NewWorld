...
model = WorldModel(embed_dim=128, num_heads=16, ff_dim=1024, num_layers=1).to(device)

# Loss & Optimizer
criterion = nn.CrossEntropyLoss()
optimizer = optim.Adam(model.parameters(), lr=0.003, weight_decay=5*1e-5)

# Early Stopping
early_stopping_patience = 20000  # Số epoch chờ trước khi dừng nếu không cải thiện
...
for epoch in range(num_epochs):
    ...
    # Log Loss lên TensorBoard và vào file
    ...
    if avg_valid_loss < best_valid_loss:
        ...
        if patience_counter >= early_stopping_patience:
            log_to_file("Early stopping triggered. Training stopped.")
